...
from .evaluate_patient_dp import dependencies as previous_dependencies

PREVIOUS=CODE/'_reports/receiver_dp8_noise_repeat_20260923_v1'
...
def run(root):
    ...
    for kind,title in (('source','BioViL'),('recipient','DenseNet')):
        ...
        for i in range(0,128,16):
            ...
            pieces.append(z.cpu())
        z=torch.cat(pieces);name=title+'_DINO_DP'
    ...
    scores,boots={},{}
    rename={'P':'P','PQ':'PQ','CLIP':'CLIP','DP1':'DP1','DP2':'DP2'}
    ...
    contrasts={}
    for title in ('BioViL','DenseNet'):
        for a,b in (('DP1','DINO_DP'),('DP2','DINO_DP'),('DINO_DP','P'),('DINO_DP','PQ')):
            ...
            contrasts[title+'_'+a+'_minus_'+b]={m:{'delta':metrics[left][m]-metrics[right][m],
                ...
                for k,m in enumerate(('AUROC','AP'))}
    evidence={arm:classify(contrasts['DenseNet_'+arm+'_minus_DINO_DP']) for arm in ('DP1','DP2')}
    positive=all(value[1]=='POSITIVE_DEVELOPMENT_SIGNAL' for value in evidence.values())
    judgment='A2_ADVANTAGE_IN_BOTH_FIXED_COMPARISONS' if positive else 'MIXED_OR_NO_CLEAR_A2_ADVANTAGE'
    names=list(scores)
    ...
    result={'status':'COMPLETE_DINO_DP_CONTROL_COMPARISON','finished':now(),
        'metrics':metrics,'comparisons':contrasts,'primary_gates':{k:v[0] for k,v in evidence.items()},'development_judgment':judgment,
        'both_A2_comparisons_positive':positive,
        'comparison_scope':'Two historical A2 noises versus one fresh DINO noise; no best-noise selection; no seed/noise-population inference',
        ...
        'A2_family_private_releases':2,'DINO_family_private_releases':1,'per_release_epsilon':8.,'per_release_delta':1e-5,
        'joint_basic_composition':{'epsilon':24.,'delta':3e-5,'optimal_accounting_claimed':False},
