"""Fixed seed101 A2(P+Q)-A2(P) comparison after the new final PNG is sealed."""
...
def dependencies():
    ...
    for p in (Path(__file__),OLD/'evaluation/result.json',OLD/'evaluation/predictions_private.npz',
              OLD/'evaluation/paired_bootstrap.npz',FIRST/'A2_result.json'):
        result[str(p)]=sha(p)
    return result
...
def main():
    ...
    require(done['completed_updates']==200 and spec['population']=='P' and spec['seed']==101,'Wrong final bank')
    ...
    for kind,title in (('source','BioViL'),('recipient','DenseNet')):
        ...
        for i in range(0,128,16):
            ...
            pieces.append(z.cpu())
        z=torch.cat(pieces);name=title+'_A2_P'
    ...
    for title in ('BioViL','DenseNet'):
        old=title+'_A2';name=old+'_PQ';index=list(previous['names']).index(old)
    ...
    for n in scores:
        if n.endswith('_PQ'):
            require(all(metrics[n][m]==old_result['metrics'][n[:-3]][m] for m in ('AUROC','AP')),'Original A2 changed')
        for k,m in enumerate(('AUROC','AP')):metrics[n][m+'_patient_cluster_95']=np.quantile(boots[n][:,k],[.025,.975]).tolist()
    contrasts={}
    for title in ('BioViL','DenseNet'):
        a=title+'_A2_PQ';b=title+'_A2_P'
        contrasts[title+'_PQ_minus_P']={m:{'delta':metrics[a][m]-metrics[b][m],
    ...
    result={'status':'COMPLETE_MATCHED_A2_PQ_VS_P','finished':now(),'metrics':metrics,'comparisons':contrasts,
        'seed':101,'new_banks':1,'updates':200,'images':128,'target_population_new':'P','target_population_reference':'P+Q',
        'interval_scope':'2000 paired V-patient draws conditional on the fixed two banks; no synthesis-seed uncertainty',
        ...
        'synthesis_seconds':{'A2_P':read(root/'A2_P_result.json')['cumulative_worker_seconds'],
                             'A2_PQ':read(FIRST/'A2_result.json')['cumulative_worker_seconds']},
