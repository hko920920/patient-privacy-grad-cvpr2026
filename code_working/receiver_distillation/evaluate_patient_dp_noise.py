...
PREVIOUS=CODE/'_reports/receiver_dp8_first_20260923_v1'
...
def run(root):
    ...
    for kind,title in (('source','BioViL'),('recipient','DenseNet')):
        ...
        for i in range(0,128,16):
            ...
            pieces.append(z.cpu())
        z=torch.cat(pieces);name=title+'_DP2'
    ...
    scores,boots={},{}
    rename={'P':'P','PQ':'PQ','CLIP':'CLIP','DP1':'DP8'}
    ...
    contrasts={}
    for title in ('BioViL','DenseNet'):
        for a,b in (('DP1','P'),('DP2','P'),('DP2','DP1'),('DP2','PQ'),('DP2','CLIP')):
            ...
            contrasts[title+'_'+a+'_minus_'+b]={m:{'delta':metrics[left][m]-metrics[right][m],
                ...
                for k,m in enumerate(('AUROC','AP'))}
    gates,judgment=classify(contrasts['DenseNet_DP2_minus_P'])
    first_gates,first_judgment=classify(contrasts['DenseNet_DP1_minus_P'])
    require(first_judgment==previous['development_judgment'],'Prior judgment changed')
    names=list(scores)
    ...
    result={'status':'COMPLETE_DP_NOISE_REPEAT_COMPARISON','finished':now(),
        'metrics':metrics,'comparisons':contrasts,'primary_gates':gates,'development_judgment':judgment,
        'first_DP_judgment':first_judgment,'both_DP_positive':first_judgment==judgment=='POSITIVE_DEVELOPMENT_SIGNAL',
        'two_release_point_mean':{t:{m:float(np.mean([metrics[t+'_DP1'][m],metrics[t+'_DP2'][m]])) for m in ('AUROC','AP')} for t in ('BioViL','DenseNet')},
        'mean_scope':'descriptive mean of two bank metrics, not an ensemble or noise-population confidence interval',
        ...
        'A2_family_private_releases':2,'per_release_epsilon':8.,'per_release_delta':1e-5,
        ...
        'receiver':'DenseNet adaptive development; BioViL included in synthesis',
