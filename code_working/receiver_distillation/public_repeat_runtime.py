"""Seed202 P-only adapter; the verified training, export and resume math is unchanged."""
...
def validate_job(job):
    require(job['schema']=='receiver.public-a2-seed202-bank/v1','Wrong public repetition job')
    require(job['population']=='P' and job['source_set']=='A2','P-only A2 required')
    require(job['seed']==job['template_seed']==202 and job['condition_seed']==101,'Seed domains changed')
    ...
    for key in ('a1_handoff','second_handoff'):
        ...
        h=read(job[key]);require(h['population']=='P' and h['privacy']=='PUBLIC_P_ONLY','Wrong population')
    return (0,)*64+(1,)*64
