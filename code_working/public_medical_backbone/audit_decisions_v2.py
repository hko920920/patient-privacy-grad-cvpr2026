...
def audit(out):
    ...
    result = dict(schema='public-medical-decision-independent-audit/v1',
                  ...
                  amendment='v2 changes only relative-path hash lookup, preserves first failed audit; no arithmetic or original source changes.',
                  previous_audit_source_sha256=digest(Path(__file__).with_name('audit_decisions.py')),
                  previous_failed_audit_sha256=digest(out / 'decision_verification_attempt1_failed.json'),
    ...
    try:
        ...
        for phase, expected_status in (
            ...
            for path in (out / 'contract.json', manifest_path, report_path):
                bound(path, verified['input_sha256'][path.relative_to(out).as_posix()])
            ...
            for row in manifest:
                ...
                for key, sha_key in [('path', 'sha256'), ('image_path', 'image_sha256')]:
                    ...
                    bound(artifact, verified['input_sha256'][artifact.relative_to(out).as_posix()])
            ballots = {}
            scopes = {}
            ...
            if phase == 'selection':
                ...
                bound(out / 'selection_decision.json', verified['input_sha256']['selection_decision.json'])
