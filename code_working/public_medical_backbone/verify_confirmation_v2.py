...
def verify(out):
    ...
    dev_run=read(dev/'execution.json')
    # diffusers serializes this set-derived metadata as an unordered list.
    # Match its names including multiplicity; every other config value stays exact.
    current_config=dict(run['scheduler_config']);prior_config=dict(dev_run['scheduler_config'])
    current_names=current_config.pop('_use_default_values');prior_names=prior_config.pop('_use_default_values')
    ch.require(isinstance(current_names,list) and isinstance(prior_names,list) and sorted(current_names)==sorted(prior_names),'default metadata names unchanged')
    ch.require(current_config==prior_config,'all scheduler parameter values unchanged')
    ...
    result=dict(status='PASS_RESERVED_CONFIRMATION_SAVED_TENSORS',complete=True,checks=ch.count,seconds=time.perf_counter()-started,
        ...
        correction=dict(kind='set-derived _use_default_values metadata ordering',
            original_code_sha256=sha(Path(__file__).with_name('verify_confirmation.py')),
            original_failure_sha256=sha(out/'verification_attempt1_failed.json'),
            configuration_utils_sha256=sha(ROOT/'base_gate/.venv/Lib/site-packages/diffusers/configuration_utils.py'),
            numeric_tolerances_changed=False,generated_data_changed=False,model_rerun=False))
