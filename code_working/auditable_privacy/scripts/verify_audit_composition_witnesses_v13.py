"""Separately recompute the fixed Audit composition witnesses.

This verifier intentionally does not import the production claim validator or
the witness builder.  In an anonymous package, the internal construction
protocol is represented only by its frozen digest and byte count; every
scientific witness input remains directly reopened.
"""
...
EXPECTED_PROTOCOL_SHA256 = (
    "2d4ad49db0808510c0943c9ddeb0bb10f8bdff93574420b60f1b363e2d2b366d"
)
EXPECTED_PROTOCOL_BYTES = 11621
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "separate_verification_v13.json"
        ),
    )
    ...
    protocol_path = resolve_bound_path(report["protocol"]["path"])
    protocol_present = protocol_path.is_file()
    protocol_hash = (
        sha256_file(protocol_path) if protocol_present else None
    )
    check(
        "protocol_binding",
        report["protocol"]["path"].endswith(PROTOCOL_NAME)
        and report["protocol"]["sha256"] == EXPECTED_PROTOCOL_SHA256
        and (
    ...
    for source in report["sources"]:
        ...
        if Path(source["path"]).name == PROTOCOL_NAME and not path.is_file():
            source_results.append(
                source["bytes"] == EXPECTED_PROTOCOL_BYTES
                and source["sha256"] == EXPECTED_PROTOCOL_SHA256
            )
        else:
            actual = {
                "exists": path.is_file(),
    ...
    output = {
        "schema_version": "audit_composition_witnesses_v13_verification.1",
        "passed": passed,
