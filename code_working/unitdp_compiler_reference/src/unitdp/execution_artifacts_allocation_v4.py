"""Deterministic artifact writer for owner random-allocation V4 runs."""
...
PUBLIC_BUNDLE_SCHEMA_ALLOCATION_V4 = (
    "unitdp.owner_random_allocation_run_bundle_public.v4"
)
...
class WrittenAllocationExecutionArtifactsV4:
    model_path: Path
...
def write_allocation_execution_artifacts_v4(
    result: OwnerRandomAllocationExecutionResultV4,
    output_dir: str | Path,
    *,
    include_private_manifest: bool = False,
    overwrite: bool = False,
) -> WrittenAllocationExecutionArtifactsV4:
    """Write a non-pickle model, public run record, and linked bundle."""

    if not isinstance(result, OwnerRandomAllocationExecutionResultV4):
        raise TypeError(
            "write_allocation_execution_artifacts_v4 requires "
            "OwnerRandomAllocationExecutionResultV4"
        )
    result.assert_model_integrity()
    target = Path(output_dir)
    target.mkdir(parents=True, exist_ok=True)
    model_path = target / "model_random_allocation_v4.json"
    public_execution_path = (
        target / "public_execution_random_allocation_v4.json"
    )
    public_bundle_path = (
        target / "public_bundle_random_allocation_v4.json"
    )
    private_manifest_path = (
        target / "private_execution_random_allocation_v4.json"
    ...
    bundle: dict[str, Any] = {
        "schema_version": PUBLIC_BUNDLE_SCHEMA_ALLOCATION_V4,
        "visibility": "public",
    ...
    return WrittenAllocationExecutionArtifactsV4(
