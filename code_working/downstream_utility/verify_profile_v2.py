...
Original classifier gate is FAILED and preserved; this review verifies the limited correction, not a retrospective pass.
"""
...
def verify(out=OUT):
    ...
    for arm in c['classifier_arms']:
        ...
        for a,b in zip(aa['trace'],bb['trace']):
            ...
            check(a['roles'][:16]==['synthetic_public']*16,'Reproduce recorded v1 wrong public source')
            ...
            second='synthetic_public' if arm in ('R0','R1') else 'private' if arm=='Dreal' else 'synthetic_'+dict(S0='backbone',S1='public',S2='private_only',S3='pooled')[arm]
    ...
    cost=dict(generation512_compute_seconds=128*sum(x['mean_seconds'] for x in generation_timing.values()),
        ...
        note='Rough architecture throughput only: original7-step runs used incorrect shared synthetic-public inputs. Not a valid seven-arm cost comparison. Excludes model IO, full validation/statistics.')
    result=dict(status='MIXED_GENERATION_PASS_ORIGINAL_CLASSIFIER_SOURCE_FAIL_REPAIR_LIMITED',complete=True,seconds=time.perf_counter()-tick,
        ...
        generation_decodes=31,original_invalid_classifier_updates=98,original_wrong_input_exact_replays=7,total_classifier_updates=100,max_BCE_error=max_loss_error,
        ...
        limitation='Original98 classifier updates have wrong public source. Corrected all-arm batch plans and only two corrected S1 updates verified; complete corrected seven-arm replay remains unexecuted. No efficacy test.',
        verifier_sha256=sha(Path(__file__)),contract_sha256=sha(out/'contract.json'))
    repair=out/'classifier_repair_v2';rc=read(repair/'contract.json')
    for p,h in rc['source_sha256'].items():check(sha(p)==h,'Repair source binding')
    rr=read(repair/'result.json');check(rr['total_optimizer_updates']==100 and rr['corrected_optimizer_updates']==2,'Total budget')
    plans=read(repair/'all_arm_batch_plan.json');check(len(plans)==49,'49 corrected batch plans')
    for row in plans:
        arm=row['arm'];expected='public' if arm in ('R0','R1') else 'private' if arm=='Dreal' else 'synthetic_'+dict(S0='backbone',S1='public',S2='private_only',S3='pooled')[arm]
        check(row['roles']==['public']*16+[expected]*16,'Corrected actual source plan')
        check(sum(row['labels'])==16 and sum(row['labels'][:16])==8,'Corrected class counts')
        for j,iid in enumerate(row['image_ids']):
            source=byid[iid] if iid in byid else gen_byid[iid]
            check(int(source['label'])==row['labels'][j],'Corrected batch label provenance')
            if iid in byid:check(source['role']==row['roles'][j] and source['patient_id']==row['patient_ids'][j],'Corrected real provenance')
            else:check('synthetic_'+source['method']==row['roles'][j],'Corrected synthetic provenance')
    for step in range(7):
        selected={r['arm']:r for r in plans if r['step']==step}
        check(all(r['image_ids'][:16]==selected['R1']['image_ids'][:16] for r in selected.values()),'Corrected shared real draws')
        check(selected['R0']['image_ids']==selected['R1']['image_ids'],'Corrected real-only pair')
    a=read(repair/'S1_0.json');b=read(repair/'S1_1.json')
    check(a['steps']==b['steps']==1,'Two corrected updates')
    check(a['trace'][0]['image_ids']==next(r['image_ids'] for r in plans if r['arm']=='S1' and r['step']==0),'Actual corrected inputs')
    for k in ('input_sha256','logits','loss','roles','image_ids','labels'):check(a['trace'][0][k]==b['trace'][0][k],'Corrected exact trace replay')
    wa=torch.load(repair/'S1_0.pt',map_location='cpu',weights_only=True);wb=torch.load(repair/'S1_1.pt',map_location='cpu',weights_only=True)
    check(all(torch.equal(wa[k],wb[k]) for k in wa),'Corrected actual weight replay')
    check(state_sha(wa)==a['final_state_sha256'],'Corrected state digest')
    for run in (a,b):
        tr=run['trace'][0];x=np.asarray(tr['logits']);y=np.asarray(tr['labels'])
        check(abs(float(np.mean(np.logaddexp(0,x)-y*x))-tr['loss'])<=2e-6,'Corrected BCE')
    result.update(corrected_namespace_batch_plans=49,corrected_exact_single_step_replays=1,
        corrected_seven_arm_training_replay_complete=False,original_classifier_gate='FAIL',
        generator_gate='PASS',repair_source_mapping='PASS_49_CPU_PLANS_AND_TWO_S1_UPDATES',
        seconds=time.perf_counter()-tick)
    with (out/'verification_v2.json').open('x',encoding='utf-8') as f:json.dump(result,f,indent=2);f.write('\n')
