...
class DiffusersDDIMConfig:
    ...
    xformers: bool = False
    # Optional exact-artifact loading controls.  Existing PP-Mark experiments
    # remain backward compatible, while integrated evidence runs can pin a
    # revision/variant and prohibit network fallback.
...
class DiffusersDDIMInverter:
    def __init__(self, cfg: DiffusersDDIMConfig):
        ...
        if cfg.disable_safety_checker:
            # DDIM inversion consumes an already selected image and never uses
            # the generation safety-checker output.  Explicitly disabling both
            # components also avoids an unpinned feature-extractor dependency.
            kwargs["safety_checker"] = None
            kwargs["feature_extractor"] = None
            kwargs["requires_safety_checker"] = False
        if cfg.variant is not None:
            kwargs["variant"] = cfg.variant
        elif use_sdxl and cfg.torch_dtype in {"float16", "bfloat16"}:
            kwargs["variant"] = "fp16"
        try:
            pipe = pipe_cls.from_pretrained(cfg.model_id, **kwargs)
        except (TypeError, ValueError):
            # An explicitly requested variant is an identity constraint.  Do
            # not silently load a different artifact if it is unavailable.
            if cfg.variant is not None:
                raise
        ...
        self.load_spec = {
            ...
            "use_safetensors": bool(cfg.use_safetensors),
            "local_files_only": bool(cfg.local_files_only),
            "disable_safety_checker": bool(cfg.disable_safety_checker),
        }
