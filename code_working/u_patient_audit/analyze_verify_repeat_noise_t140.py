...
def verify(directory):
    ...
    need(c['analysis_code_sha256']==digest(__file__),'frozen verifier source')
    ...
    raw_hashes={};values={};shared={};max_mse_error=0.;reused=0
    for (m,i),row in index.items():
        ...
        for d,summary in zip(raw['draws'],row['draws']):
            ...
            a=float(alphas[140]);expected=math.sqrt(a)*z.double().numpy()+math.sqrt(1-a)*eps.double().numpy()
            close(noisy.numpy(),expected,'independent noising',rtol=2e-6,atol=1e-7)
    ...
    write(directory/'verification.json',v)
...
def analyze(directory,c,index,values):
    out=directory/'analysis_v1';out.mkdir(exist_ok=False);start=time.perf_counter()
    ...
    write(out/'protocol.json',dict(contract_sha256=read(directory/'protocol.json')['contract_sha256'],
        ...
        code_sha256=digest(__file__),policy=analysis_policy(),raw_verification_sha256=digest(directory/'verification.json')))
