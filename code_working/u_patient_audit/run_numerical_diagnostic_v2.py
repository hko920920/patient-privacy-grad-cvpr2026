"""V2 numerical diagnostics preserving raw checkpoint/legacy comparison evidence."""
...
def run_precision(precision,checkpoint,image_id,cache,out):
    ...
    raw_checkpointing=dict(on=dict(value=cell0['value'],base_loss=cell0['base_loss'],
        target_loss=cell0['target_loss'],gradient=cell0['gradient'],cells=[cell0]),off=serial(off))
    legacy=None;raw_legacy=None;legacy_f=legacy_b=0
    if precision=="fp16":
        ...
        raw_legacy=dict(value=old_value,base_loss=old_base,target_loss=old_target,gradient=serial(old_gradient))
    ...
    result=dict(status="NUMERICAL_MEASUREMENTS_COMPLETE",precision=precision,trace=trace,
        ...
        raw_checkpointing=raw_checkpointing,raw_legacy=raw_legacy,
...
def main():
    ...
    parser.add_argument("--output-tag",default="numerical_v2")
    args=parser.parse_args()
    assert args.output_tag.replace("_","").isalnum()
    out=RUN/"verification_20260914"/args.output_tag
    if out.exists():raise RuntimeError("immutable numerical output already exists; use a new tag")
    ...
    contract=dict(schema="cvpr-u-actual-numerical/v2",raw_comparison_evidence=True,checkpoint_sha256=digest(checkpoint),
