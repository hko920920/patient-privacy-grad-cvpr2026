"""Fixed U-only CDI feature extraction with immutable per-image CPU packets.
...
OUTPUT = RUN / "baseline_screen_20260914/cdi_u_cohort_v1"
...
REUSE_IDS = ("00014393_001.png", "00014393_004.png")
...
def validate(contract):
    required = dict(schema="cdi-u-cohort-contract/v1", current_step=2,
        eval_roles=["fit", "selection"], scenario="U", models=list(MODELS),
    ...
    for role, count in (("fit", 80), ("selection", 40)):
        ...
        for patient in ids:
            ...
            selected.extend(sorted((r for r in rows if r["record_role"] == "U_observed"), key=lambda r: r["image_id"]))
    ...
    for model in MODELS:
        ...
        for row in selected:
            ...
            assert row["image_id"] not in manifest and exposures.get(row["image_id"], 0) == 0
            records[model].append(dict(model=model, scenario="U", patient_id=patient,
                image_id=row["image_id"], eval_role=row["eval_role"], assignment_group=row["assignment_group"],
                record_role="U_observed", member=member, image_member=0, actual_training_exposures=0,
...
def main():
    ...
    if OUTPUT.exists():
        ...
        _new_json(OUTPUT / "protocol.json", {
            "schema": "cdi-u-cohort-execution/v1", "current_step": 2, "started_utc": now(),
            ...
            "scope": "fixed U-only fit80 and previously observed selection40; feature extraction, not attack fitting",
        })
    ...
    try:
        ...
        assert all(r["record_role"] == "U_observed" and r["actual_training_exposures"] == 0 for r in results)
        ...
        summary = {"schema": "cdi-u-cohort-execution-result/v1",
            ...
            "scope": "complete fixed U extraction; selection is existing development data; no efficacy or failure-cause conclusion"}
