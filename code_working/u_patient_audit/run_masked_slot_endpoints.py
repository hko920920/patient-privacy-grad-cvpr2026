...
OUTPUT = BASE / "masked_slot_endpoints_v1"
CONTRACT = BASE / "masked_slot_endpoints_contract_v1.json"
...
def prepare(args):
    ...
    paths += [Path(__file__), ROOT/"u_patient_audit/models.py", ROOT/"u_patient_audit/common.py",
        ...
        ROOT/"u_patient_audit/analyze_verify_masked_slot_endpoints.py",
    ...
    c = dict(schema="masked-slot-endpoint-contract/v1", created_utc=now(), policy=policy(),
...
def validate(args):
    ...
    require(c["schema"]=="masked-slot-endpoint-contract/v1" and c["policy"]==policy(), "fixed endpoint policy")
...
def execute(args,c,states,sources,rows):
    ...
    new_json(out/"protocol.json",dict(schema="masked-slot-endpoint-execution/v1",contract=c,
