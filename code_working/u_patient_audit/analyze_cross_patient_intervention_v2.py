...
PREDECESSOR=Path(__file__).with_name('analyze_cross_patient_intervention.py')
PREDECESSOR_SHA='8de498cd7bb5e2e683902b917340708ddbec866dcf3bdf702e21836e9d37c1a6'
...
def run(args):
    ...
    assert correction['source_v1_sha256']==sha(PREDECESSOR)==PREDECESSOR_SHA
    assert correction['source_v2_sha256']==args.expected_code_sha256 and correction['analysis_policy_sha256']==POLICY_SHA
    assert correction['schema']=='cross-intervention-dino-metadata-correction/v1'
    ...
    bindings[str(CORRECTION.resolve())]=sha(CORRECTION);bindings[str(PREDECESSOR.resolve())]=PREDECESSOR_SHA
    new(dest/'protocol.json',dict(schema='cross-patient-existing-scorer-analysis/v2',policy_sha256=POLICY_SHA,
        source_code_sha256=args.expected_code_sha256,scorer_helper_sha256=HELPER_SHA,
        metadata_correction_sha256=sha(CORRECTION),predecessor_code_sha256=PREDECESSOR_SHA,
            ...
            for arm in c['arms']:
                ...
                for spec in specs:
                    ...
                    metadata=dict(arm=arm,method=method,scenario=scenario,clean_fixed_C_primary=clean,
                        ...
                        p_in_prior_coefficient_fit=bool((parameter and p in parameter['fitting_patient_ids']) or spec['kind']=='unchanged'),
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--run-dir',type=Path);p.add_argument('--output-tag',default='existing_scorer_analysis_v2')
