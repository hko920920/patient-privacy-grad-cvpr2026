"""Revision 2: repair missing noised import; preserve v1 failure/output history.
...
from .verify_cdi_u_cohort import (
    ...
    check_noising,check_prediction,check_loss_gradient,recurrence,noised,digest,inside,
    read_csv,write_new,load_json,tight)
V1_SHA="d548f006e3b1dea32c6ecf32e60577261d1eb51203cf4ead41bd1459786a57e9"
...
def self_test():
    ...
    # Audit every function/nested function's referenced globals, not just syntax.
    ...
    def walk(scope):
        if scope.get_type()=="function":
            for symbol in scope.get_symbols():
                ...
                if symbol.is_global() and symbol.is_referenced() and name not in globals() and not hasattr(builtins,name):
                    unresolved.append((scope.get_name(),name))
        for child in scope.get_children():walk(child)
    walk(table)
    require(not unresolved,"undefined function globals: "+repr(unresolved))
    require(digest(ROOT/"u_patient_audit/verify_cdi_base_selection.py")==V1_SHA,"v1 source preserved")
    # This is an existing TARGET fixture with adapted guard-only metadata, not
    # evidence that its tensors were produced by a base model. No new data/GPU.
    kp=RUN/"baseline_screen_20260914/cdi_kernel_v1"
    raw=torch.load(kp/"raw.pt",map_location="cpu",weights_only=True)
    cache=torch.load(RUN/"cache/cache.pt",map_location="cpu",weights_only=True)
    rows=[r for r in load_json(kp/"results.json") if r["scenario"]=="U"]
    require(len(rows)==2,"two previously verified U fixtures")
    for row in rows:
        iid=row["image_id"]
        adapted=dict(row,model="base",member=0,incremental_patient_member=0)
        answer=verify_base_image(adapted,raw[iid],cache["latents"][iid],{},kp)
        original=verify_image(row,raw[iid],cache["latents"][iid],{},kp)
        require(answer["features_cpu_float64"]==original["features_cpu_float64"],"all26 fixture arithmetic exact")
        require(answer["prior_SecMI_states_exact"] is None,"fixture does not claim base SecMI equality")
    require(not torch.cuda.is_initialized(),"CPU only")
    print("PASS_GLOBAL_DEPENDENCIES_AND_TWO_EXISTING_TARGET_FIXTURES_THROUGH_BASE_GUARD; no base performance read")
...
def main():
    ...
    require(digest(ROOT/"u_patient_audit/verify_cdi_base_selection.py")==V1_SHA,"frozen predecessor preserved")
    d=a.run_dir.resolve();out=d/"verification_v2.json";vp=d/"verification_v2_protocol.json"
    require(not out.exists() and not vp.exists(),"immutable verification")
    names=["protocol.json","execution.json","results.json","verification.json","verification_protocol.json"]
    if a.analysis_tag:
    ...
    write_new(vp,dict(verification_revision=2,predecessor_source_sha256=V1_SHA,source_sha256=digest(__file__),helper_sha256=HELPER_SHA,input_sha256=hashes,
