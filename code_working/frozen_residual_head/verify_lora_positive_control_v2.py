...
ORIGINAL_VERIFIER_SHA256="cecc4d4a076e93e78349ceb2538dc872cda63bf0e3d3501d30c6ba50f026e486"
AMENDMENT_NAME="verification_amendment_v2.json"
...
def verify_amendment(check,out,contract):
    amendment=read(out/AMENDMENT_NAME)
    bound=amendment["bound_sha256"]
    for path,digest in bound.items():
        check.require(sha(path)==digest,"amendment preserved source/evidence "+path)
    original=Path(__file__).with_name("verify_lora_positive_control.py").resolve()
    current=Path(__file__).resolve()
    bridge=current.with_name("run_lora_positive_control_v2.py")
    for path in (original,current,bridge,out/"contract.json",out/"replay_verification.json",out/"replay_verification_protocol.json"):
        check.require(bound[str(path)]==sha(path),"required amendment source/failure link "+str(path))
    check.require(amendment["verifier_v2_sha256"]==sha(current),"v2 verifier bound after original replay")
    check.require(amendment["bridge_runner_v2_sha256"]==sha(bridge),"separate v2 bridge entry bound")
    check.require(amendment["created_after_original_replay"] is True
                  and amendment["numeric_algorithms_changed"] is False
                  and amendment["gpu_replay_repeated"] is False,"honest correction timing/scope")
    check.require(isinstance(amendment["reason"],str) and bool(amendment["reason"]),"documented correction reason")
    failure=read(out/"replay_verification.json")
    check.require(failure["complete"] is False and failure["phase"]=="replay"
                  and failure["code_sha256"]==ORIGINAL_VERIFIER_SHA256
                  and failure["error"]=="AssertionError('replay_pipeline_fp16: actual dtype timesteps')",
                  "preserved original dtype failure")
    check.require(failure["input_sha256"]["contract.json"]==sha(out/"contract.json"),"original failed contract preserved")
    for name,digest in failure["input_sha256"].items():
        check.require(sha(out/name)==digest,"original failed evidence preserved "+name)
    parse=lambda path:{node.name:ast.dump(node,include_attributes=False)
        for node in ast.parse(Path(path).read_text(encoding="utf-8")).body if isinstance(node,(ast.FunctionDef,ast.AsyncFunctionDef))}
    before,after=parse(original),parse(current)
    for name in ("cfg","ddim","independent_alphas","schedule","png_pixels","historical_seed","image_from_packet"):
        check.require(before[name]==after[name],"unchanged numerical function AST "+name)
...
def check_bridge_pair(check,earlier,later,transition):
    if transition=="exact_manual_replay":
        ...
        for key in earlier:
            if key=="timesteps":
                integer_equal(check,later[key],earlier[key],"pipeline/manual exact integer timestep values")
            else:
    ...
    integer_equal(check,later["timesteps"],earlier["timesteps"],"bridge unchanged scheduler timestep values "+transition)
...
def verify_contract(check,out,contract):
    ...
    original=Path(__file__).with_name("verify_lora_positive_control.py").resolve()
    check.require(sources[str(original)]==sha(original)==ORIGINAL_VERIFIER_SHA256,"original verifier remains prebound and unchanged")
    verify_amendment(check,out,contract)
...
def verify_runtime_and_execution(check,out,contract,phase):
    ...
    if phase=="bridge":
        ...
        check.require(bridge["contract_sha256"]==sha(out/"contract.json")
                      ...
                      and bridge["replay_verification_sha256"]==sha(out/"replay_verification_v2.json"),"bridge output hash links")
...
def verify(out,phase):
    ...
    result_path=out/("replay_verification_v2.json" if phase=="replay" else "independent_verification_v2.json")
    protocol_path=out/("replay_verification_protocol_v2.json" if phase=="replay" else "independent_verification_protocol_v2.json")
    if result_path.exists() or protocol_path.exists():
        raise FileExistsError("Preserve previous independent verification attempt")
    names=["contract.json","manifest_replay.json","runtime_replay.json","execution_replay.json",AMENDMENT_NAME]
    if phase=="bridge":
        names.extend(["manifest_bridge.json","execution_bridge.json","replay_verification_v2.json"])
    bound={name:sha(out/name) for name in names}
    source=sha(__file__)
    protocol={"schema":"independent-lora-positive-control-verification/v2","phase":phase,
        "created_utc":datetime.now(timezone.utc).isoformat(),"code_sha256":source,"initial_input_sha256":dict(bound),
        "numeric_policy":"Original numerical source bound before replay; v2 signed-integer timestep representation correction finalized after original replay and before v2 verification, bound by explicit amendment. CFG/cast/image and paired pipeline/manual floating paths exact. Each DDIM update checked against independent FP64 equation using gamma_32 of its actual FP16 or FP32 dtype and 32 smallest subnormal units; CPU dtype replay exactness is separately reported.",
    ...
    try:
        ...
        if phase=="bridge":
            prior=read(out/"replay_verification_v2.json")
            ...
            for case in contract["cases"]:
                ...
                integer_equal(check,final["timesteps"],previous["timesteps"],"M1/current-base exact timetable values "+case["case_id"])
...
def self_test():
    ...
    integer_equal(checker,schedule(config).astype(np.int32),schedule(config).astype(np.int64),
                  "int32/int64 are identical timestep values")
    invalid=[schedule(config).astype(np.float32),schedule(config).astype(np.uint32),
             np.concatenate((np.array([-1],np.int32),schedule(config)[1:].astype(np.int32))),
             np.concatenate((np.array([1000],np.int32),schedule(config)[1:].astype(np.int32))),
             schedule(config).astype(np.int32)+1]
    rejected=0
    for bad in invalid:
        try:
            integer_equal(Check(),bad,schedule(config),"invalid synthetic integer schedule")
        except AssertionError:
            rejected+=1
    checker.require(rejected==len(invalid),"float/unsigned/out-of-range/changed-value timestep arrays rejected")
    ...
    return {"status":"PASS_SYNTHETIC_CFG_DDIM_AND_HISTORICAL_SEEDS","checks":checker.count,
            "invalid_timestep_cases_rejected":rejected}
