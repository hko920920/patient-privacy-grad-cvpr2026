# V2 bridge only: consume corrected independent verifier; original source/contract/results remain frozen.
...
def validate(out):
    ...
    amendment=read(out/"verification_amendment_v2.json")
    for path,digest in amendment["bound_sha256"].items():require(sha(path)==digest,"Changed amendment input: "+path)
    require(sha(__file__)==amendment["bridge_runner_v2_sha256"],"V2 runner changed")
...
def bridge(out):
    ...
    verify=read(out/"replay_verification_v2.json");require(verify["complete"] and verify["status"].startswith("PASS"),"Verified original replay required")
    ...
    save(out/"execution_bridge.json",dict(completed=True,utc=now(),seconds=time.perf_counter()-start,
        ...
        replay_verification_sha256=sha(out/"replay_verification_v2.json"),
...
if __name__=="__main__":
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument("phase",choices=("bridge",))
